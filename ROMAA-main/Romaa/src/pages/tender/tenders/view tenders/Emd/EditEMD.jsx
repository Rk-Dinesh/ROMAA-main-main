...
const onSubmit = async (data) => {
  ...
  try {
   ...
   const alreadyApproved = emdData.proposals.some((p) => p.status === "APPROVED");
...
console.log(alreadyApproved);
    ...
    const res = await axios.put(`${API}/emd/updateproposal/${tender_id}/${item.proposal_id}`, data);
  ...
  return (
    <div className="fixed inset-0 grid justify-center items-center backdrop-blur-xs backdrop-grayscale-50 drop-shadow-lg z-20 font-roboto-flex">
      <div className="mx-2 shadow-lg py-2 dark:bg-layout-dark bg-white rounded-md w-[420px]">
        <div className="grid">
          <button onClick={onclose} className="place-self-end cursor-pointer rounded-full">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="grid px-6 py-6 space-y-4">
             <InputField
                            ...
                            />
              <InputField label="Level" name="level" register={register} errors={errors} placeholder="Enter level" />
            </div>
